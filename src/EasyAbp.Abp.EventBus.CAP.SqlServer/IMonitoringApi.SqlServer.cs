...
{
    internal class SqlServerMonitoringApi<TDbContext> : DapperRepository<TDbContext>, IMonitoringApi
...
select count(Id) from {_recName} with (nolock) where StatusName = N'Failed';";
        ...
        private int GetNumberOfMessage(IDbConnection connection, string tableName, string statusName)
        {
            ...
            var count = connection.ExecuteScalar<int>(sqlQuery, new { state = statusName }, DbTransaction);
            return count;
        }
...
select [Key], [Count] from aggr with (nolock) where [Key] in @keys;";
            ...
            var valuesMap = connection
                .Query<TimelineCounter>(_options.IsSqlServer2008 ? sqlQuery2008 : sqlQuery, new { keys = keyMaps.Keys, statusName }, DbTransaction)
