...
{
    public class PostgreSqlMonitoringApi<TDbContext> : DapperRepository<TDbContext>, IMonitoringApi
        where TDbContext : IEfCoreDbContext
    {
        ...
        public async Task<MediumMessage> GetPublishedMessageAsync(long id)
        {
            ...
            return await this.DbConnection.QueryFirstOrDefaultAsync<MediumMessage>(sql, transaction: DbTransaction);
        }
...
select count(""Id"") from {_recName} where ""StatusName"" = N'Failed';";
        ...
        private int GetNumberOfMessage(IDbConnection connection, string tableName, string statusName)
        {
            ...
            var count = connection.ExecuteScalar<int>(sqlQuery, new { state = statusName }, transaction: DbTransaction);
            return count;
        }
